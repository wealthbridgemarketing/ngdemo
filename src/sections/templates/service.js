...
siteApp.controller('TemplatesCtrl',
    ...
    function($state, $scope, BaseSrvc, Breadcrumbs, LoadExternal, appContent, appTemplates, $uibModal, $compile) {
        ...
        //log([['appContent & appTemplates','i','blue'],appContent,appTemplates]);
...
siteApp.directive('contentItem',
['$state', '$compile', function($state, $compile) 
{
...
siteApp.factory('LoadExternal',
    ['$http', '$q', '$timeout', 'BaseSrvc', function($http, $q, $timeout, BaseSrvc) {
        var load = function(ajaxURI, lsKey, expDate) {
            ...
            //log([['TemplatesCtrl:LoadExternal: "'+lsKey+'" value: ','i'],data]);
            if (data !== undefined) deferred.resolve(data);
                ...
                $http.get(ajaxURI).then(function(response) {
                    ...
                    $timeout(function() {
                        //log([['TemplatesCtrl:LoadExternal: "'+lsKey+'" set in local storage: ','i'],base.ls.get('TemplatesCtrl:'+lsKey)]);
